...
class DealCurator {
  ...
  /**
   * Scrape / discover live ASINs from Amazon.in Today's Deals pages
   */
    ...
    for (const url of dealUrls) {
      try {
        ...
        if (res.ok) {
          ...
          asinMatches.forEach(m => discovered.add(m[1]));
        }
      } catch (err) {
  ...
  async runDailyCuration(customAsins = null) {
    ...
    // 3. Discover live deals from Amazon Today's Deals pages (up to 40 items)
    const liveDeals = await this.discoverLiveTodayDeals();
    liveDeals.slice(0, 40).forEach(asin => candidateAsins.add(asin));
    ...
    for (const asin of totalList) {
      try {
        ...
        const productRecord = {
          ...
          current_price: lookup.current_price, // Strict live price from Amazon only — NO random fallback
