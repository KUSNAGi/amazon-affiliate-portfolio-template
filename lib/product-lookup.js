...
class ProductLookup {
  ...
  async lookupByAsin(asin) {
    ...
    try {
      ...
      // Strict Availability & In-Stock Verification
      ...
      const isExplicitlyUnavailable = availText.includes('currently unavailable') ||
                                     availText.includes("don't know when or if this item will be back in stock") ||
                                     availText.includes('out of stock') ||
                                     (html.includes('See All Buying Options') && !hasAddToCart) ||
                                     (!hasAddToCart && html.includes('Currently unavailable'));

      if (isExplicitlyUnavailable || (!hasAddToCart && !availText.includes('in stock') && !availText.includes('left in stock'))) {
        return {
          success: false,
